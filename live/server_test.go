...
func TestServer_DashboardHTML(t *testing.T) {
	...
	for _, want := range []string{"<!DOCTYPE html>", "samber-do-auditlog", "LIVE", "skip-link", "Skip to main content", "main-content", "role=\"tab\"", "aria-selected", "aria-controls", "tabindex", "showShortcutsHelp", "Keyboard shortcuts", "ArrowRight", "ArrowLeft", "Press ? for shortcuts"} {
...
func TestServer_DashboardHTML_JavaScriptBalanced(t *testing.T) {
	...
	js := extractExecutableJS(t, rec.Body.String())
	if len(js) < 100 {
...
// type="application/json" from an HTML string.
