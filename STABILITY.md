...
| `Plugin.SetOnEvent(fn func(Event))`                             | New in 0.10.0. Late event-callback wiring. Replaces (does not chain) `Config.OnEvent`; race-safe with recording.                             |
| `Plugin.Enable()`                                               | New in 0.10.0. Late opt-in for plugins created before feature flags are parsed. Effective only before the first `Opts()` result is consumed. |
