...
- [x] ~~**Backport candidates to master**~~ DONE 2026-09-04 — **live/ snapshot/subscribe ordering fix backported** (master `live/server.go` d092158; master's live-delivery test hung 10 min on the race, verified fixed with `-race -count=5`). Correction: the html_view CSS-class constants are NOT on master (master has html.templ, not html_view.go — earlier claim unverified); they remain a candidate. Remaining candidates (classSuccess/classError constants, devShell GOEXPERIMENT hardening, live/demo coverage-exclusion parity) await the merge-direction decision.
