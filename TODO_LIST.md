...
- [ ] **Backport candidates to master (after merge direction is decided)**: html_view CSS-class constants (already committable), devShell `GOEXPERIMENT=""` hardening, coverage-exclusion for `live/demo/` parity, **live/ snapshot/subscribe ordering fix** (master subscribed after the initial snapshot; an event fired in that gap stayed invisible until the next event — fixed here in `live/server.go` 2026-09-04).
