...
- **SSE wire writer hardened against field injection**: `writeSSEEvent` strips CR/LF from `event:`/`id:` values, so a hostile service name can never inject additional SSE fields into a frame; enforced by `FuzzSSEWireRoundTrip` (frame structure + data round-trip under arbitrary payloads). `FuzzRingEventsAfter` fuzzes the reconnection replay slice (ordering, membership, eviction) under arbitrary Last-Event-ID values. 10 fuzz targets total; `scripts/verify.sh` runs the whole pre-commit + CI gate matrix locally, including per-target live fuzzing (`VERIFY_FUZZ_SECS`, 0 = off).
...
- **Cross-project feature ports from `go-workflow-auditlog`** (sibling project, same author, same patterns):
  - **Adopt `go-error-family` classification** (`classify.go`): all sentinel errors are registered into `go-error-family` families (Corruption/Rejection) via `ErrorClassifications()` and auto-registered in `init()`. Upgraded `go-error-family` from v0.8.0 to v0.9.0 (now a direct dependency).
  - **Adopt `go-atomic-write`**: replaced the custom `writeToFile()` helper in `plugin.go` with `go-atomic-write` v0.3.0 (`atomicwrite.WriteFunc`) for crash-safe file writes with fsync durability and cross-platform atomic rename.
  - **NDJSON streaming** (`stream.go`): `NDJSONStreamer` writes events as newline-delimited JSON in real time via `Config.OnEvent`. Thread-safe with configurable auto-flush and buffer size. `CreateNDJSONStreamer` opens a file for tailing. Uses standard `encoding/json` (no `jsontext` dependency, respecting the project's `encoding/json/v2` exclusion policy).
...
- **`go-sse` made public**: Module published at v0.2.0. `replace` directive removed from `go.mod`. Both `go-sse` and `go-ndjson` are now public — zero `replace` directives remain.
...
- **go-output testhelpers workarounds replaced with version pins**: Removed the two consumer-side `replace` directives and the golangci-lint allow-list. Explicit indirect requirements now select the published `testhelpers/v0.31.1` and `testhelpers/graphtest/v0.31.1` tags over upstream's broken zero pseudo-versions; these modules are graph-verification dependencies and contribute no compiled packages to this project.
- **Atomic file writes via `go-atomic-write`**: `writeToFile()` in `plugin.go` now delegates to `go-atomic-write` v0.3.0 (`atomicwrite.WriteFunc`) for TOCTOU-safe writes with fsync durability and cross-platform atomic rename. Removes the custom temp-file+rename implementation. Added to depguard `main` allow-list.
- **`go-error-family` upgraded v0.8.0 → v0.9.0**: Now a direct dependency (was indirect via go-output). Added to depguard `main` and `tests` allow-lists.
- **Diagram Write methods accept variadic options**: `WriteMermaid`, `WriteDOT`, `WritePlantUML`, `WriteD2`, and their Plugin wrapper methods now accept `...DiagramOption`. Backward compatible (existing calls without options compile unchanged). Export methods (`ExportToMermaid`, etc.) also accept `...DiagramOption`.
- **Table WriteTable/WriteTableString accept variadic options**: Both Report and Plugin methods now accept `...TableOption` for column selection. Backward compatible.
...
- **Docs-accuracy audit**: corrected linter count (108 → 109), `GraphStyle` → `NodeStyle` (renamed in go-output v0.30.0), Go version (1.26.3 → 1.26.4 in `go.mod`), coverage gate threshold (95% → 94% in 4 locations to match actual CI config), `MigrateReport` description, `RenderTableData` → `RenderTable`, stale test/parallelism counts, release versioning pattern (`v0.0.x` → `v0.x.y`).
