...
- **Diagram rendering via `go-output`**: Mermaid/PlantUML/DOT are produced by `github.com/larsartmann/go-output` renderers (`graph.MermaidRenderer`, `plantuml.PlantUMLDiagram`, `graph.DOTRenderer`). `diagram.go` builds `[]output.GraphNode`/`[]output.GraphEdge` from the report; each `Write*` method configures its renderer (`SetNodes`/`SetEdges`/`DedupEdges`), then `writeRendered()` does the single `Render()+Write`. Escaping is go-output's validated `escape` package (`SlugifyID`+`MermaidID` for node IDs, `MermaidText`/`PlantUML`/`DOT` for labels). Compiled go-output packages: root + enum + envdetect + escape + graph + plantuml (+ `go-branded-id`, `x/term`); `delimited`/`markdown`/`tree`/`testhelpers` are graph-verification-only, zero packages linked. Adoption logged in `docs/research/go-output-adoption-review.md` §9.
...
- **Adding a 4th diagram format** = construct the go-output renderer in a new `Write<Format>` method, then `SetNodes(buildDiagramNodes(r))` / `SetEdges(buildDiagramEdges(r))` / `DedupEdges()` / `writeRendered(w, renderer)`. Node IDs via `diagramNodeID(scopeID, serviceName)` (SlugifyID+MermaidID); labels via `serviceLabel(svc)` (with type icon) or bare `dep.ServiceName` for external deps.
...
- **Test helpers**: `mkEvent` (replay_test.go, `auditlog_test` package) and `mkRegEvent` (cli_integration_test.go, `main` package) create standard event structs. `mkEventWithDur` extends `mkEvent` with `DurationMs`. Use these instead of inline struct literals to keep art-dupl clone-free.
...
- **Duplication policy**: art-dupl at `-t 30` with `--semantic` (matches buildflow). **Zero clones in production AND test code.** Test helpers (`mkEvent`, `mkEventWithDur`, `mkRegEvent`, `rootRef`, assertion wrappers) centralize struct creation to prevent duplication. Accepted categories: (1) idiomatic Go benchmark loops, (2) single-line assertion wrappers (`assertServiceCount`, `assertEventCount`, etc.), (3) parallel type-different helpers (`assertIntField`/`assertStringField`), (4) demo pairs (`do.ProvideValue` vs `do.OverrideValue`), (5) `mkEvent()`/`mkEventWithDur()`/`mkRegEvent()` calls (function calls to a shared helper are not duplication). Shared production helpers: `getOrCreateServiceRecord(evt)` (replay path), `recordDependencyFromStack`, `buildServiceDeps`, `depRecToRef`, `sortServiceInfos`, `buildScopeTreeFromMeta` (generic). Enum metadata uses map-based lookups (`eventTypeMetaTable`, `providerTypeMeta`, `serviceStatusIcons`) — single source of truth for Label/Icon/Color.
