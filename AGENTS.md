...
- **go-output v0.31.1 upgrade + release bug workaround**: go-output was upgraded from v0.30.1 → v0.31.1 (all sub-modules in lockstep via `go mod edit`). v0.31.1 adds `d2Quote()` which fixes D2 hex-color/label quoting (previously `#e8a838` was treated as a comment by D2). **Release bug**: go-output v0.31.1 tags were published with broken pseudo-version references (`v0.0.0-00010101000000-000000000000`) for `testhelpers` and `testhelpers/graphtest` in go.mod — the local `replace` directives were not stripped before tagging. **Workaround**: two targeted `replace` directives in go.mod redirect the broken pseudo-versions to the real `testhelpers/v0.31.1` and `testhelpers/graphtest/v0.31.1` tags. These are whitelisted in `.golangci.yml` `gomoddirectives.replace-allow-list`. Remove both replaces + the allow-list entry once go-output fixes their release process (the sibling project `go-workflow-auditlog` has the same issue documented in `docs/status/2026-07-23_04-11_d2-fmt-fix-completion-and-self-review.md`).
- **Cross-project learnings from `go-workflow-auditlog`** (sibling project, same author, same patterns): The workflow auditlog project is a more mature reference for several patterns this project could adopt: (1) **`go-error-family` classification** — workflow-auditlog directly imports `go-error-family` and classifies all sentinel errors into Families (Corruption/Rejection/Transient/Infrastructure) via `classify.go` with auto-registration in `init()`; this project has `go-error-family` only as a transitive dep (v0.8.0) and classifies nothing. (2) **`go-atomic-write`** — workflow-auditlog uses `github.com/larsartmann/go-atomic-write` for crash-safe file exports; this project uses its own `writeToFile()` helper in `plugin.go` (functional but not extracted into a shared lib). (3) **NDJSON streaming** — workflow-auditlog has `NDJSONStreamer` (`stream.go`) for real-time event streaming via `Config.OnEvent` with `WithAutoFlush`/`WithBufferSize`; this project lacks streaming. (4) **Diagram direction** — workflow-auditlog supports `WithDirection(output.Direction)` across all 4 diagram formats; this project doesn't. (5) **Table column selection** — workflow-auditlog has `WithColumns(TableColumn...)` with 10 selectable columns; this project has fixed columns. These are documented in `go-workflow-auditlog/AGENTS.md` as reference implementations.
