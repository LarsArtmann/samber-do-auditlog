...
ndjson.go           — ReadEvents: stdlib NDJSON reader (bufio line scanner + per-line decode) with domain-specific event validation
...
diagram.go          — Self-contained stdlib diagram pipeline (port of the go-output escape+graph+plantuml+d2 renderers, byte-identical output): diagramNodeStyle, warmAmberNodeStyle, node/edge builders, renderGraphDiagram, writeRendered
mermaid.go          — Mermaid flowchart export (stdlib renderer on the diagram.go port, code-fence off)
plantuml.go         — PlantUML component diagram export (stdlib renderer)
dot.go              — Graphviz DOT digraph export (stdlib renderer, graphID="do_auditlog", rankdir=LR)
d2.go               — D2 diagram export (stdlib renderer, dedupGraphEdges helper)
html.go             — HTML export entry points (Plugin.WriteHTML delegates to Report.WriteHTML)
html_view.go        — html/template views for the self-contained static report (warm-amber identity, 5-tab layout, keyboard nav; replaces html.templ — NO code generation on this branch)
loader.go           — LoadReport: stdlib auto-detecting loader (Format + Detect port; routes JSON via MigrateReport, NDJSON via ReadEvents + ReplayEvents)
...
tree.go             — ASCII tree + HTML nested-list tree via stdlib treeNode renderers (replaces go-output tree renderers)
table.go            — Service summary table export via stdlib TableFormat renderers: 5 formats (table/json/csv/tsv/markdown)
schema.go           — go:embed of schema/report.schema.json + JSONSchema() accessor + //go:generate directive
design_tokens.go    — DesignTokensCSS: canonical CSS design tokens consumed by html_view.go (static report) + live/base_css.go (dashboard)
shared_components.go — SharedComponentCSS: canonical keyboard-nav overlay CSS (skip-link, kbd-help dialog) consumed by html_view.go + live/base_css.go
...
ndjson.go           — ReadEvents + StreamEvents: stdlib reader + callback-based NDJSON line scanner
...
```
live/sse.go         — SSE wire format: sseEvent{Name,Data,ID,Retry}, writeSSEEvent, line split/join/keyed helpers, sseStream per-connection writer (mutex-serialized send/flush/heartbeat), lastEventIDFromRequest
live/broadcaster.go — broadcaster: map-based subscriber fan-out with per-subscriber buffer (128 events, drop-on-overflow), graceful drain (Draining poll), broadcasterHealth{Draining,SubscriberCount,BufferSize}
live/replay.go      — eventRingBuffer: FIFO replay for reconnecting clients (default 1000 events, eventsAfter(lastID))
live/hub.go         — Hub facade: OnEvent (marshal→sseEvent, seq IDs), Subscribe/Unsubscribe/Done, SignalComplete/IsComplete, ClientCount, ReplayStore()/BufferedEventCount(), Shutdown/Health pass-through. Defines sseEventType constant.
live/server.go      — HTTP server with 6 endpoints (dashboard, report JSON, SSE events, health, export NDJSON, export HTML), configurable prefix, CORS middleware, graceful shutdown. SSE handler subscribes the client BEFORE rendering/sending the snapshot (race fix 4adc545), drains event bursts (drainEvents), sends datastar-patch-elements + patch-signals via the stdlib wire writer.
live/fragments.go   — Precomputed view rows (serviceRow/eventRow/scopeNodeView etc.) built from Report + pure helpers (humanizeDuration, providerIcon, computeWaveformMarks, …) — same pattern as the static report's html_view.go
live/fragments_html.go — html/template fragment sources (10 defines incl. recursive scopeNode) rendered via renderFragment; element IDs + datastar attributes are byte-compatible with master's templ output (the browser contract)
live/dashboard.go   — Dashboard HTML shell (datastar attributes: data-signals, data-init, data-bind, data-show, data-class:active, data-on:click). Embeds datastar.js via go:embed. Contains renderEventFilterChips().
...
live/base_css.go    — Live dashboard CSS: composes shared design tokens (auditlog.DesignTokensCSS + auditlog.SharedComponentCSS) + live-specific aliases + base component styles
live/doc.go         — Package doc comment
live/server_test.go — External tests: server lifecycle, SSE streaming, handler edge cases, hub unit tests, CORS, export endpoints; uses a ~60-line stdlib SSE wire client (replaces go-sse/ssetest)
live/transport_internal_test.go, live/hub_internal_test.go, live/fragments_internal_test.go, live/fragments_coverage_internal_test.go — Internal tests for transport, hub, ring buffer, and view-builder internals
live/demo/          — Self-contained real-time demo (registers services with delays, shows dashboard updating live). Demo services implement do.Healthchecker. Excluded from the coverage gate.
```
...
### Shared infrastructure: standard library only (this branch)

This branch has **zero third-party runtime deps**: go.mod requires only
`samber/do/v2` plus `invopop/jsonschema` (schema tooling, `cmd/`-only). The
infrastructure that master delegates to external modules is ported locally:

- **SSE transport** (master: `go-sse`): `live/sse.go` (wire format, per-connection
  stream writer, heartbeats, Last-Event-ID) + `live/broadcaster.go`
  (drop-on-overflow fan-out, graceful drain) + `live/replay.go` (FIFO ring buffer).
- **NDJSON read/write + format detection** (master: `go-ndjson`): stdlib `bufio`
  line scanner + `encoding/json` in `ndjson.go`/`loader.go`.
- **Diagram/table/tree renderers** (master: `go-output`): self-contained port in
  `diagram.go` (byte-identical output for the inputs auditlog produces) plus
  stdlib renderers in `table.go`/`tree.go`.
- **Atomic file writes** (master: `go-atomic-write`): stdlib temp-file + rename in
  `plugin.go` `writeToFile()`.

A `go.work` workspace at the parent directory may still link sibling projects
for local development, but nothing in this module imports them.
...
### Go 1.26.7 toolchain pin (MASTER ONLY — not this branch)

> **go1.23-compat note**: this branch pins **Go 1.23** — go.mod `go 1.23`, ci.yml `go-version: "1.23"` in all 7 jobs, flake.nix `GOTOOLCHAIN = "go1.23.12"` (devShell + coverage + auditlog apps). The bump-in-same-commit rule applies identically here with 1.23 in all three places.
...
GitHub Actions workflow at `.github/workflows/ci.yml` runs on every push and PR with 7 parallel jobs (branch truth 2026-09-04):

- **test**: go-version drift guard, `go vet`, `go build`, `go test -race` with a coverage profile, and a **coverage gate** that fails if non-`example/`/`cmd/`/`live/demo/` statement coverage drops below 94% (exclusions single-sourced in `scripts/coverage-exclusions.txt`).
- **lint**: binary cache + `golangci-lint v2.1.6` (pinned: newest release installable under Go 1.23), `config verify` then `run --timeout=10m`.
- **vulncheck**: `govulncheck@v1.1.4` (newest release whose go directive 1.22 is satisfiable by the 1.23 toolchain) with `continue-on-error: true` — go1.23 is EOL, every finding is a stdlib advisory only newer toolchains fix, and the module has zero third-party runtime deps; findings stay visible in the job log. Master keeps a strict gate.
- **mod-tidy**: runs `go mod tidy` and fails if `go.sum` drifts from the committed version.
- **stale-generation**: runs `go generate ./...`, fails on diff (schema generator only on this branch — no templ).
- **actionlint**: workflow lint via `go install github.com/rhysd/actionlint/cmd/actionlint@v1.7.7` (newest release installable under Go 1.23).
- **example-smoke**: builds and runs the `example/` binary as a smoke test (replaces master's goreleaser job, which is not wired on this branch).
...
- **Version-skew ledger (MASTER ONLY — not this branch)**: `live/fragments_templ.go`-era `live/fragments.go:181` carries a `//nolint:goconst` (provider-type literals) needed by master CI's pinned golangci-lint v2.12.2 but flagged as unused by local v2.13.1's nolintlint. Retire it when the CI pin bumps ≥ 2.13. The three other 2026-08 sites (`loader.go:50`, `stream.go:129`, `live/server_test.go:684`) were unused directives and were removed by the lint-fix commit `cf5f205` (2026-09-01). On THIS branch the equivalent ledger is the **Version-skew ledger (branch)** bullet near the top of this file (noctx findings + lint-cache warning).
...
- **go-output version history (MASTER ONLY — not this branch)**: upgraded through v0.30.1 → v0.31.1 → v0.32.0 → … → v0.37.0 (all sub-modules in lockstep). v0.31.1 added `d2Quote()` (fixes D2 hex-color/label quoting — previously `#e8a838` was treated as a comment by D2). The v0.31.1 published manifests contained broken zero pseudo-versions for `testhelpers` and `testhelpers/graphtest` (local `replace` directives were not stripped before tagging), requiring consumer-side explicit indirect pins at v0.31.1; corrected manifests exist from v0.32.0 onward. The indirect pins in master's `go.mod` track the go-output family version. This branch carries the stdlib diagram port instead (see "Shared infrastructure" above).
- **Cross-project feature ports from `go-workflow-auditlog`** (sibling project, same author, same patterns; items (1) and (2) describe MASTER — this branch carries stdlib equivalents): Five patterns were ported from the sibling project: (1) **`go-error-family` classification** (master only; this branch has no classify.go — sentinels are plain wrapped errors) — registers all sentinel errors into Families (Corruption/Rejection) with auto-registration in `init()`. (2) **`go-atomic-write`** (master only; this branch uses a stdlib temp-file + rename `writeToFile()` in `plugin.go`) — `atomicwrite.WriteFunc` (v0.5.0) for crash-durable atomic writes; audit exports use plain writes (no read-modify-write cycle). (3) **NDJSON streaming** — `stream.go` provides `NDJSONStreamer` for real-time event streaming via `Config.OnEvent` with `WithAutoFlush`/`WithStreamBufferSize`; uses standard `encoding/json` (no `jsontext` dependency). (4) **Diagram direction** — `diagram_options.go` provides `WithDirection(output.Direction)` across all 4 diagram formats. (5) **Table column selection** — `table_options.go` provides `WithColumns(TableColumn...)` with 10 selectable columns.
- **Datastar-powered live dashboard** (v1.0.2): The live dashboard uses [Datastar](https://data-star.dev/) for SSE transport, DOM morphing, and signal-based reactivity. The server renders HTML fragments (`live/fragments.go` view rows + `live/fragments_html.go` html/template sources) and sends them as `datastar-patch-elements` SSE events (via the stdlib wire writer's keyed-lines helper in `live/sse.go`; master used go-sse `SendLines` + `KeyedLines`). Datastar morphs the DOM by element ID, preserving focus/scroll/transitions. Client-side state (tab, search, filter, pagination) is managed via datastar signals (`data-signals`, `data-bind`, `data-show`, `data-class`, `data-on:click`) — no hand-written rendering JS. The old 977-line `dashboard.js` SSE client/rendering engine is replaced by `datastar.js` (~56KB runtime) + a ~180-line keyboard nav/export helper script. Per-row `data-signals` + `data-show` expressions enable instant client-side search/filter/pagination without server roundtrips. The SSE handler coalesces event bursts via `drainEvents` (non-blocking channel drain) before re-rendering. Reconnection sends a fresh full snapshot (all fragments) — the snapshot IS the replay; the server subscribes the client BEFORE sending the snapshot so no event falls in the gap (fix 4adc545).
