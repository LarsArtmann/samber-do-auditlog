...
table.go            — Service summary table export via go-output RenderTableData: 16+ formats
...
- **Typed identifiers / ServiceInfo split are DEFERRED to v0.3.0** — `ContainerID`/`ScopeID`/`ServiceName` named string types and splitting `ServiceInfo` into identity/lifecycle/health/graph structs were evaluated: blast radius measured at 65+ compile errors across production + tests + generated templ, with **zero existing bugs** from string/monolith usage. These are the next breaking-change batch (do together, post-v0.2.0). The full analysis lives in `docs/status/` reports.
...
- **Duplication policy**: art-dupl at `-t 15` with `--semantic` (matches `buildflow --semantic -t 15` settings; the strict `-t 30` ceiling mentioned historically was relaxed in favor of explicit per-category acceptance). **Zero harmful clones in production AND test code.** Test helpers (`mkEvent`, `mkEventWithDur`, `mkRegEvent`, `rootRef`, `assertEqual[T comparable]`, assertion wrappers) centralize struct creation and assertions to prevent drift. Accepted categories (last verified by `art-dupl --semantic --sort total-tokens -t 15`, 16 clone groups total — all intentional): (1) **idiomatic Go benchmark loops** — `for b.Loop() { _, _ = do.InvokeNamed[*Database](injector, "db") }` (4 matches in `benchmarks_test.go`); (2) **single-line assertion wrappers** — `assertServiceCount`/`assertEventCount`/`assertContainerID` and `assertDependenciesCount`/`assertServiceInvocationCount`/`assertServiceHealthCheckCount` (6 wrappers, all 3-line `t.Helper + assertEqual/assertServiceIntField + return`); (3) **assertReportValid (Fatalf) vs assertReportValidNoFatal (Errorf)** — differ only in failure mode; bool-flag param would be less readable than two clearly-named functions; (4) **demo pairs** — `do.ProvideValue` vs `do.OverrideValue` with different configs in `example/register.go:93-103`; (5) **`time.Date(...)` test data** — 12 single-line timestamp initializers across test files (each picks a distinct wall-clock time for traceable fixture data); (6) **table-driven test data** — `goldenEvent(N, offsetMs, Type, Phase, name)`/`mkEvent(N, ts, Type, Phase, name, cid, type)`/`mkRegEvent(N, ts, name, cid)` calls inside fixture slices where each row differs in numeric parameters; (7) **cross-package test fixtures** — `mkRegEvent` and `rootRef`-equivalent inline literals exist in both `cmd/auditlog` (`main` package binary tests) and the library's `auditlog_test` package — no shared import path exists, each is one site within its package; (8) **per-scenario test assertions** — e.g. `if report.DroppedEventCount != N { t.Fatalf(...) }` in different test functions with different expected values; (9) **Example function boilerplate** — `ExampleReport_WriteMermaid`/`ExampleReport_WriteD2` share buffer+error-check structure but must be standalone per Go convention. Production code refactors that eliminated harmful duplication: `fireEvent` method (7 → 1 sites for onEvent callback), `assertEqual[T comparable]` (replaced `assertIntField` + `assertStringField` with one generic function), `newFlagSet` helper (6 → 1 sites for CLI flag set creation), `printBreakdown` generic (eliminated `printStatusBreakdown`), single-pass loop in `stats.go` (3 loops → 1). Shared production helpers: `getOrCreateServiceRecord(evt)` (replay path), `recordDependencyFromStack`, `buildServiceDeps`, `depRecToRef`, `sortServiceInfos`, `buildScopeTreeFromMeta` (generic), `newFlagSet` (cmd/), `fireEvent` (hooks). Enum metadata uses map-based lookups (`eventTypeMetaTable`, `providerTypeMeta`, `serviceStatusIcons`) — single source of truth for Label/Icon/Color.
...
- **Table export** (`WriteTable`): 16+ format table export of service summary (Service, Scope, Type, Status, Invocations, Build(ms), Error) via go-output `RenderTableData`. Formats include: table, json, csv, tsv, markdown, xml, d2, yaml, html, tree, mermaid, dot, jsonl, asciidoc, toml, plantuml. Available on both `Report` and `Plugin`.
