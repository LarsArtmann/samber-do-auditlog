...
- **BuildFlow needs `GOEXPERIMENT=jsonv2`**: buildflow inherits only the parent shell's environment — it does NOT read `.env` files and has no working `env:` config key (the key is silently ignored by `config view`). Without `GOEXPERIMENT=jsonv2`, every Go step fails with `build constraints exclude all Go files in .../encoding/json/v2` (via the `go-ndjson` transitive dependency). Fix: run buildflow from a shell where direnv has loaded `.envrc` (`use flake`) or from `nix develop`. The `go-fix` step also requires the global `--fix` flag to be executable (otherwise: `no executable nodes after compilation`).
