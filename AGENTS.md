...
report.go           — Report type + query methods (ServiceByName, EventsByType, Index, etc.)
...
diagram.go          — Shared diagram formatter infrastructure (writeDiagram, mermaid/plantuml formatters)
mermaid.go          — Mermaid flowchart export (Report.WriteMermaid)
plantuml.go         — PlantUML component diagram export (Report.WritePlantUML)
