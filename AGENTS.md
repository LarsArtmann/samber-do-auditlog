...
- **Coverage: ~94%** of statements (root 95.4%, live 84.2%), 313 top-level functions (288 Test + 12 Benchmark + 5 Fuzz + 8 Example), 14 `t.Run` subtests, and 3 `b.Run` sub-benchmarks (BenchmarkBuildReport at 50/100/500 services); 297 `t.Parallel()` calls (~97% of eligible tests — only `t.Setenv()` env-var tests run sequentially).
