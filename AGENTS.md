...
History: v0.7.0 shipped with `go 1.26.5` in `go.mod` (then considered drift; 1.26.4 was canonical). v0.7.1 reverted the directive and added the `GOTOOLCHAIN` pin. The pin moved back to 1.26.5 when `go-atomic-write` v0.4.0 / `go-error-family` v0.10.0 / `go-output` v0.32.0 / `go-sse` v0.2.1 all bumped their `go` directive to 1.26.5, making it the new required minimum.
