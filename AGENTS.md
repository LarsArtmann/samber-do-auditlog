...
- **Coverage: ~95%** of statements, 167 top-level functions (146 Test + 11 Benchmark + 5 Fuzz + 7 Example), 11 `t.Run` subtests, and 3 `b.Run` sub-benchmarks (BenchmarkBuildReport at 50/100/500 services); 152 `t.Parallel()` calls (~97% of eligible tests — only `t.Setenv()` env-var tests run sequentially).
