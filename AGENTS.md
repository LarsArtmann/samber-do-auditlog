...
- **`IsHealthchecker`/`IsShutdowner` fields always `false`** — `do.ExplainServiceOutput` doesn't expose whether a service implements `do.Healthchecker` or `do.Shutdowner`. These fields exist in `ServiceInfo` and `serviceRecord` but are hardcoded to `false` in both `newServiceRecord()` and `newServiceRecordFromMeta()`. Remove them or populate via reflection/upstream PR.
