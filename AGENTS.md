...
```
plugin.go        — Public API: New(), Opts(), Report(), Export*(), Events(), MigrateReport
recorder.go      — Core state machine: event capture, invocation stack, service aggregation
types.go         — All domain types (Event, ServiceInfo, Report, ReportOption, etc.)
...
example_test.go  — Godoc examples (6 Example* functions)
fuzz_test.go     — HTML fuzz test (FuzzPluginHTML)
```
