...
The `replace` directives in `go.mod` (`go-sse => ../go-sse`, `go-ndjson => ../go-ndjson`)
remain because both modules are private; they will be removed once the repos are made public.

### Shared infrastructure: `go-ndjson`

NDJSON read/write and format-detection logic delegate to the external
[`github.com/larsartmann/go-ndjson`](https://github.com/larsartmann/go-ndjson) module (currently
private, local replace `=> ../go-ndjson`). The local `loader.go` and `ndjson.go` re-export the
public API so existing callers are unaffected. The `go-ndjson` module itself uses standard
`encoding/json` (not `encoding/json/v2`).
...
The requirement exists because `go-output` (used for diagram/table rendering) and `go-branded-id`
(transitive dependency of `go-output`) intentionally use `encoding/json/v2` features
...
- **Coverage: ~91%** of statements (root 93.9%, live 76.6%), 303 top-level functions (278 Test + 12 Benchmark + 5 Fuzz + 8 Example), 14 `t.Run` subtests, and 3 `b.Run` sub-benchmarks (BenchmarkBuildReport at 50/100/500 services); 288 `t.Parallel()` calls (~97% of eligible tests — only `t.Setenv()` env-var tests run sequentially).
