...
live/replay.go      — eventStore adapter (legacy, retained for tests): adapts plugin.Events() to sse.EventStore
live/server.go      — HTTP server with 6 endpoints (dashboard, report JSON, SSE events via datastar wire format, health, export NDJSON, export HTML), configurable prefix, CORS middleware, graceful shutdown + broadcaster drain
live/fragments.go   — Server-side HTML fragment rendering for all dashboard sections (stats, legend, waveform, services table, events table, scope tree, footer) — replaces client-side rendering
live/dashboard.go   — Dashboard HTML renderer (embeds datastar.js + CSS + minimal keyboard nav JS, injects route prefix)
live/dashboard.css  — Dashboard stylesheet (warm amber theme, responsive layout)
live/dashboard.js   — Minimal keyboard nav + export helpers (~180 lines). Replaces the old 977-line SSE client/rendering engine. Datastar.js handles SSE, DOM morphing, and signal reactivity.
live/datastar.js    — Datastar v1.0.2 runtime (~56KB), embedded via go:embed. Handles SSE parsing, DOM morphing by element ID, and reactive signal evaluation.
