...
- **Duplication policy**: art-dupl is run at `-t 15` semantic threshold. Accepted remaining clones are documented inline in `docs/status/` reports. Categories accepted as intentional: (1) idiomatic Go benchmark loops (hot-loop is the benchmark subject), (2) named single-line assertion wrappers with semantic intent (e.g. `assertServiceCount`, `assertEventCount`, `assertDependenciesCount`), (3) parallel type-different helpers (`assertIntField`/`assertStringField`), (4) demo pairs that showcase paired samber/do APIs (`do.Provide` vs `do.ProvideTransient`, `do.ProvideValue` vs `do.OverrideValue`), (5) table-driven test signature repetition (function-type field in struct literal), (6) cross-file idioms with context-specific error messages (e.g. `DroppedEventCount` checks with different expected values), (7) `Report.Validate()` success check pattern (3 sites → `assertReportValid` helper), (8) "exactly one service" check pattern (4 sites → `requireOneService` helper). **Goal is zero harmful duplication, not zero report lines** — extraction must improve maintainability without complicating the abstraction.
