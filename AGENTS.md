...
- **Diagram escaping**: `sanitizeDiagramID` (shared by both formatters) collapses separators (`- / . * [ ]`) to `_` then strips any non-`[a-zA-Z0-9_]` char (returns `"node"` if empty). Mermaid labels go through `mermaidLabel` (`"`→`'`, `[]`/`{}`→`()`, `\n`→`<br>`); PlantUML labels through `plantumlLabel` (`"`→`'`). Hardens against service names containing brackets/quotes. Reviewed adopting `github.com/larsartmann/go-output` for this — declined (breaking output format + 11-module dep bloat); see `docs/research/go-output-adoption-review.md`.
