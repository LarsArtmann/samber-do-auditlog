...
| **live/ sub-package tests**  | 73 test functions (38 server, 31 fragment helpers, 4 replay) covering dashboard HTML, health, report, 404, SSE snapshot/live/complete/fan-out/heartbeat, graceful shutdown, client count, server lifecycle, nil-plugin edge cases, CORS, export endpoints, hub lifecycle, buffer overflow | `live/server_test.go`, `live/fragments_internal_test.go`, `live/replay_test.go` |
...
| Feature                 | Status    | Note                                                                         |
| ----------------------- | --------- | ---------------------------------------------------------------------------- |
| Live dashboard coverage | Below bar | 78.3% vs the 90% internal goal; templ fragment renderers are the biggest gap |
...
_Last verified against the codebase on 2026-09-01. Coverage: 95.2% gate (root 94.9%, live 78.3%, testhelpers 91.1%; generated `*_templ.go` excluded)._
