...
| **Graceful shutdown**            | `Server.Shutdown(ctx)` stops HTTP server and waits for active SSE connections to drain                                       | `live/server.go`; `TestServer_GracefulShutdown`    |
| **Health endpoint**              | `GET {prefix}/api/health` returns uptime, connected client count, total events, and dropped events                           | `live/server.go`; `TestServer_HealthEndpoint`      |
