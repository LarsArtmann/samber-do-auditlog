...
| **Multi-format table export**     | `Report.WriteTable(writer, format, opts)` outputs service summary in 5 formats (table, json, csv, tsv, markdown) via `TableFormat`     | `table.go`                  |
| **Self-contained HTML export**    | `Plugin.ExportToHTML(path)` and `Plugin.WriteHTML(w)` render a single-file report                                                     | `html.go`, `html_view.go`   |
...
| Feature                     | Description                                                                                                                                                                                                                       | Verified                    |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| **Dark-themed dashboard**   | Custom CSS color scheme, IBM Plex Mono / Space Grotesk typography                                                                                                                                                                 | `html.go`, `html_view.go`  |
| **Header with metadata**    | Container ID, schema version, export timestamp                                                                                                                                                                                    | `html.go`, `html_view.go`  |
| **Stats cards**             | Services, scopes, events, dependencies, total build, errors, health-check summary                                                                                                                                                 | `html.go`, `html_view.go`  |
| **Lifecycle waveform**      | Event timeline strip colored by event type, height scaled by duration                                                                                                                                                             | `html.go`, `html_view.go`  |
| **Services table**          | Columns for service, type, scope, status, order, invocations, build/shutdown ms, deps, dependents, health                                                                                                                         | `html.go`, `html_view.go`  |
| **Scopes tree**             | Collapsible tree with service counts and provider icons                                                                                                                                                                           | `html.go`, `html_view.go`  |
| **Dependency graph**        | Sugiyama layered DAG layout with barycenter crossing reduction, cubic Bézier edges, pan/zoom/fit, click-to-highlight, mouse wheel and touch support                                                                               | `html.go`, `html_view.go`  |
| **Timeline tab**            | Dual horizontal bar chart of build vs shutdown durations                                                                                                                                                                          | `html.go`, `html_view.go`  |
| **Events table**            | Event list with sequence, time, type badge, provider badge, phase, scope, service, duration, error                                                                                                                                | `html.go`, `html_view.go`  |
| **Live service search**     | Debounced filter on service name/scope/type                                                                                                                                                                                       | `html.go`, `html_view.go`  |
| **Event type filter chips** | All / registration / invocation / shutdown / health_check buttons                                                                                                                                                                 | `html.go`, `html_view.go`  |
| **Keyboard navigation**     | Skip link, ARIA tablist (Arrow/Home/End + roving tabindex), `?` help dialog with focus trap + restoration, `/` focus search, `e` toggle errors-only, Esc closes overlays, Enter/Space on sortable column headers with `aria-sort` | `html.go`, `html_view.go`  |
| **Pagination**              | "Show first N" with "Show all" button for services (50) and events (100)                                                                                                                                                          | `html.go`, `html_view.go`  |
| **Error tooltips**          | Click error status badges to show the full error message                                                                                                                                                                          | `html.go`, `html_view.go`  |
| **Type metadata injection** | `BuildTypeMetadata()` JSON is injected into the page as the single source of truth for icons and labels                                                                                                                           | `metadata.go`, `html_view.go` |
| **Responsive layout**       | Mobile-friendly padding, media queries, reduced-motion support                                                                                                                                                                    | `html.go`, `html_view.go`  |
| **Content Security Policy** | CSP meta tag with `base-uri 'none'`, `frame-ancestors 'none'`, and inline style/script allowances                                                                                                                                 | `html.go`, `html_view.go`  |
| **XSS hardening**           | All user-controlled strings escaped via `esc()` before DOM insertion                                                                                                                                                              | `html.go`, `html_view.go`  |
...
| **IO-boundary `string()` convention** | External library calls (csv, fmt) wrap typed values with `string()` at the IO boundary                                                         | `csv.go`, `d2.go`, `diagram.go`                                    |
...
| Feature                          | Description                                                                                                                                                                                                            | Verified                                           |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| **SSE event hub**                | `Hub` is a facade over the stdlib `broadcaster`, broadcasting `auditlog.Event` as wire-ready SSE events to all connected clients with a 128-event buffer per client. Events carry the auditlog `Sequence` as the SSE `id:` field for reconnection replay | `live/hub.go`, `live/broadcaster.go`               |
...
| **SSE connection lifecycle**     | `sseStream` (stdlib `net/http` + `http.Flusher`) manages SSE headers, mutex-serialized writes, flush, heartbeat comment frames, and disconnect detection via the request context                                       | `live/sse.go`, `live/server.go`                    |
| **SSE reconnection replay**      | Reconnecting clients with `Last-Event-ID` receive missed events replayed from the ring buffer, followed by a fresh full snapshot (datastar semantics)                                                                  | `live/replay.go`; `TestServer_SSE_ReconnectReplay` |
| **SSE ring buffer**              | FIFO ring buffer stores recent events for reconnection replay. `ReplayBufferSize` config (default 1000), `ReplayStore()` and `BufferedEventCount()` on `Hub`                                                           | `live/replay.go`, `live/hub.go`                    |
| **Zero-dependency transport**    | The whole SSE stack (wire format, fan-out with drop-on-overflow, graceful drain, ring replay) is implemented on the standard library only — no go-sse, no templ, works on the Go 1.23 floor                           | `live/sse.go`, `live/broadcaster.go`, `live/replay.go` |
...
| Feature                      | Description                                                                                                                                                                                                                                                                               | Verified                                                                        |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **GitHub Actions CI**        | 7 parallel jobs: `go vet`, `go build`, race-detector tests with coverage gate, golangci-lint, govulncheck, `go mod tidy` drift, generated-code drift, actionlint, goreleaser config check. All actions pinned to commit SHAs for supply-chain security.                                   | `.github/workflows/ci.yml`                                                      |
...
| **golangci-lint config**     | `.golangci.yml` defines lint rules for the project (107 enabled linters)                                                                                                                                                                                                                  | `.golangci.yml`                                                                 |
| **Generated-code check**     | CI runs `go generate ./...` and fails on drift, keeping `schema/report.schema.json` in sync with the Go types (no templ on this line)                                                                                                                                                     | `.github/workflows/ci.yml`                                                      |
| **Fuzz tests**               | 8 targets: HTML XSS (service names, error messages, dep chains), `MigrateReport` integrity, Mermaid/PlantUML special chars, filter-option combinations, NDJSON parsing resilience, plus `FuzzMultiWriter`, `FuzzNDJSONStreamer`, `FuzzClassifyAdversarialChains`                          | `fuzz_test.go`, `filter_fuzz_test.go`, `fuzz_crossproject_test.go`              |
...
| **Test parallelism**         | 453 `t.Parallel()` calls; only `t.Setenv()` env-var tests run sequentially                                                                                                                                                                                                                | all `*_test.go`                                                                 |
| **Type metadata assertions** | `TestBuildTypeMetadata` directly asserts provider/status icons, labels, and colors                                                                                                                                                                                                        | `metadata_test.go`                                                              |
| **live/ sub-package tests**  | Transport wire-format, ring buffer, broadcaster fan-out/drain, hub replay, fragment view builders, and an external HTTP suite (dashboard, health, report, 404s, SSE snapshot/live/complete/fan-out/replay/heartbeat, no-flusher, CORS, exports, JS balance, server lifecycle, nil-plugin) | `live/server_test.go`, `live/*_internal_test.go`                                |
...
| Feature                 | Status    | Note                                                                         |
| ----------------------- | --------- | ---------------------------------------------------------------------------- |
| Live dashboard coverage | On bar | 94.2% under the repo-wide 94% gate (stdlib port, no generated code excluded) |
| Coverage gate margin    | Headroom  | 95.2% vs 94% threshold; adding code without tests erodes the 1.2pp margin    |
...
_Last verified against the codebase on 2026-09-04 (go1.23-compat). Coverage gate: 94.9% total, live/ 94.2% (demo/ excluded; no generated files). CI: all jobs green via workflow_dispatch on this branch._
