...
| **Content Security Policy** | CSP meta tag with `default-src 'none'`, `base-uri 'none'`, and inline style/script allowances (no `frame-ancestors` — header-only directive, enforced by the live server instead)                                                                                 | `html.templ`                |
...
| **go-sse full adoption**         | Uses `Stream`, `Broadcaster[T]`, `Replay`, `EventStore`, `Shutdown`, `Health` from `go-sse` (v0.5.1). No hand-rolled fan-out or connection management code remains                                                     | `live/hub.go`, `live/server.go`, `live/replay.go`  |
...
| Feature                      | Description                                                                                                                                                                                                                                                                               | Verified                                                                        |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **GitHub Actions CI**        | 8 parallel jobs: test (vet + build + race tests + coverage gate), lint, govulncheck, mod-tidy drift, stale-generation drift (incl. committed-generated guard), actionlint, goreleaser config check, example-smoke (23-feature self-check). All actions pinned to commit SHAs for supply-chain security.                                    | `.github/workflows/ci.yml`                                                      |
...
| **golangci-lint config**     | `.golangci.yml` defines lint rules for the project (108 enabled linters)                                                                                                                                                                                                                  | `.golangci.yml`                                                                 |
...
| **Test parallelism**         | 482 `t.Parallel()` calls across all test packages; only `t.Setenv()` env-var tests run sequentially                                                                                                                                                                                       | all `*_test.go`                                                                 |
| **Type metadata assertions** | `TestBuildTypeMetadata` directly asserts provider/status icons, labels, and colors                                                                                                                                                                                                        | `metadata_test.go`                                                              |
| **live/ sub-package tests**  | 79 test functions (41 server, 31 fragment helpers, 3 fragment render, 4 replay) covering dashboard HTML + CSP, health, report, 404, SSE snapshot/live/complete/fan-out/heartbeat, graceful shutdown, client count, server lifecycle, nil-plugin edge cases, CORS, export endpoints, hub lifecycle, buffer overflow                 | `live/server_test.go`, `live/fragments_internal_test.go`, `live/fragments_render_internal_test.go`, `live/replay_test.go` |
...
| Feature                 | Status    | Note                                                                         |
| ----------------------- | --------- | ---------------------------------------------------------------------------- |
| Live dashboard coverage | Below bar | 79.8% vs the 90% internal goal; templ fragment renderers are the biggest gap |
| Coverage gate margin    | Headroom  | 95.5% vs 94% threshold; adding code without tests erodes the 1.5pp margin    |
...
_Last verified against the codebase on 2026-09-11. Coverage: 95.5% gate (root 94.9%, live 79.8%, testhelpers 91.1%; generated `*_templ.go` excluded)._
