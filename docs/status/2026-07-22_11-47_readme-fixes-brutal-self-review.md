...
The three original P0 fixes (Mermaid quoted syntax, API link restoration, Loading & Migrating section) are in the current README. ~~The two defects this report self-identified as newly introduced remain **unfixed** as of 2026-07-24~~ **UPDATE (2026-07-24 later session):** Both defects were FIXED in the 16:51 session and recorded in CHANGELOG `[Unreleased]`: (1) the Loading & Migrating code block now uses `os.ReadFile("old-report.json")` and `os.Open("events.ndjson")` — fully compilable; (2) the "zero exemptions" claim corrected to "minimal exemptions for tests and tooling". The Mermaid example node-ID simplification note remains an open item in TODO_LIST.
