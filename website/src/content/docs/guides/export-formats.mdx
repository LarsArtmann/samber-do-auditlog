  ...
  "services": [
    {
      ...
      "dependencies": [{ "scope_name": "[root]", "service_name": "*main.Database" }]
    }
  ]
}
```
...
Every format above also has a `Write*String` variant returning the output as a `string` — see the [API Reference](/api-reference/).

## Where to go next

- [Filtered Reports](/guides/filtered-reports/) — export only the slice you need
- [Loading & migrating reports](/api-reference/#package-functions) — `LoadReport`, `ReplayEvents`, `MigrateReport`
- [Live Dashboard](/guides/live-dashboard/) — the same exports, one click from the browser
- [Performance](/guides/performance/) — what each format costs at export time
